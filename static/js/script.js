...
class ContentViewFactory{
    static createContentViewFromElement(contentView) {
        ...
        if (themeType == "I") {
            if (locationType == "R") {
                return new RoomInformationContentView(contentView);
            } else {
                return new LocationInformationContentView(contentView);
            }
        } else if (themeType == "H") {
            return new LocationHistoryContentView(contentView);
        } else if (themeType == "B") {
            return new BedsInformationContentView(contentView);
        } else {
            return new LocationsInformationContentView(contentView);
        }
    }
...
class ContentView {
    ...
    static fillLocationSelect(locations, locationSelect, locationType, user_data_representation) {
        if (locations != null && locations.length != 0) {
            ...
            ContentView.addEmptyOptionToSelect(locationSelect);
        }
    }
    ...
    static setRectangleClassAndTextForSex(locationData, sexRectangle, sexSpan) {//TODO auslagern in extra vorlklasse
    ...
    static addEmptyOptionToSelect(select) {// TODO add error message
        const newOption = document.createElement("option");
    ...
    onDragStart() {
        ContentView.draggedContentView = this;
        this.#contentView.classList.add("dragged-content-view");
    }

    onDragEnter() {
        if (this != ContentView.draggedContentView) {
            // delete the content view from the array
            ContentView.contentViews.splice(ContentView.draggedContentView.order, 1);
            if (ContentView.draggedContentView.order < this.#order) {
                ContentView.parent.insertBefore(ContentView.draggedContentView.contentView, this.#contentView.nextElementSibling);
                // add content view to the right place
                ContentView.contentViews.splice(this.#order, 0, ContentView.draggedContentView);
            } else {
                ContentView.parent.insertBefore(ContentView.draggedContentView.contentView, this.#contentView);
                // add content view to the right place
                ContentView.contentViews.splice(this.#order, 0, ContentView.draggedContentView);
            }
            //Change ordering
...
class BedsInformationContentView extends ContentView {
    ...
    fillContentViewDataDivWithData(data) {
        ...
        bedsData.forEach(bedData => {
            ...
            ContentView.setRectangleClassAndTextForSex(bedData, sexRectangle, sexSpan);
